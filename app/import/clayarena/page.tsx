...
export default function ClayArenaImportPage() {
  ...
  useEffect(() => {
    ...
    if (sharedUrl && !forwardedUrlLoadedRef.current) {
      forwardedUrlLoadedRef.current = true;
      setUrl(sharedUrl);
      void findResult(undefined, sharedUrl);
    }
  // A forwarded link is parsed once; local edits should not repeat the request.
  ...
  async function findResult(event?: React.FormEvent, requestedUrl = url) {
    event?.preventDefault(); setBusy(true); setMessage(null); setCandidates([]); setSavedSessionId("");
    try {
      const response = await fetch("/api/clayarena/parse", { method: "POST", headers: { "Content-Type": "application/json", ...(await authorization()) }, body: JSON.stringify({ url: requestedUrl }) });
  ...
  return <main>
    <form className="card" onSubmit={findResult}>
      <p className="eyebrow"><Link href="/import">Import</Link> / ClayArena</p><h2>Import from ClayArena</h2>
      <p>Paste a results link. Review the result before saving.</p>
      ...
      <div className="btns"><button disabled={busy}>{busy ? "Finding result..." : "Find result"}</button></div>
    ...
    {candidate ? <section className="card">
      ...
      <div className="compactSummaryGrid"><span><strong>Shooter</strong> {candidate.shooterName}</span><span><strong>Score</strong> {candidate.ownScore}/{candidate.totalTargets ?? "?"}</span><span><strong>Date</strong> {candidate.date || "Not provided"}</span><span><strong>Discipline</strong> {candidate.discipline}</span><span><strong>Venue</strong> {candidate.venue || "Not provided"}</span><span><strong>Placement</strong> {candidate.placement ?? "Not provided"}</span>{candidate.winningScore !== null ? <span><strong>Winning score</strong> {candidate.winningScore}</span> : null}</div>
      {candidate.warnings.map((warning) => <div className="notice small" key={warning}>{warning}</div>)}
      <details className="detailAccordion" open={!candidate.date || !candidate.totalTargets}>
        <summary>Correct result details</summary><div className="detailAccordionBody">
      ...
      <p className="small muted">Category: {candidate.category || "Not provided"} · Source: ClayArena</p>
