...
import { competitionCoverage, competitionFinalizeBlockReason, competitionIsReadOnly, parseCompetitionStatus, type CompetitionStatus } from "@/lib/scoreSheets/competitionLifecycle";
...
export default function ScoreSheetEditor({ kind }: { kind: "training" | "competition" }) {
  ...
  useEffect(() => {
    ...
    if (draft && !draft.synced && localDraftHasMeaningfulSetup(draft)) {
      ...
      setRecoveryPrompt(null);
      setHasUnsyncedLocalDraft(false);
      if (quickStartPreset === "compak") {
        setTitleAutoGenerated(true);
        setSessionDate(todayValue());
        setLocation("");
        setDiscipline(COMPAK_SPORTING);
        setSessionType(kind);
  ...
  useEffect(() => {
    if (!localDraftLoaded || recoveryPrompt || recoveryAutosavePaused || lifecycleReadOnly) return;
  ...
  async function reopenCompetition() {
    ...
    setCompetitionStatus("live"); setCompetitionReopenedAt(row.competition_reopened_at); setCompetitionReopenCount(row.competition_reopen_count); setLastKnownServerUpdatedAt(row.updated_at); setHasUnsyncedLocalDraft(false); setLocalSaveStatus("synced"); setSavedMessage("Result reopened for correction."); setSaving(false);
  }
...
{!isCompetition && <>
        ...
        <div className="btns stackedOnMobile">
          ...
          {isCompetition && !lifecycleReadOnly && !isNew && <button type="button" className="secondary" onClick={() => { setAllowIncompleteFinalization(false); setShowFinalizationReview(true); }} disabled={Boolean(finalizationBlockReason)}>Review &amp; finalize</button>}
          <Link href="/dashboard" className="button secondary">
            Dashboard
          </Link>
        </div>

        {showFinalizationReview && isCompetition && !lifecycleReadOnly && <div className="modalBackdrop" role="presentation"><section className="card" role="dialog" aria-modal="true" aria-labelledby="finalize-heading"><h3 id="finalize-heading">Review &amp; finalize</h3><div className="resultsSummaryMeta"><div><span>Competition</span><strong>{title}</strong></div><div><span>Date</span><strong>{formatTitleDate(sessionDate)}</strong></div><div><span>Discipline</span><strong>{discipline}</strong></div><div><span>Scored targets</span><strong>{reviewCoverage.scored}</strong></div><div><span>Unscored targets</span><strong>{reviewCoverage.unscored}</strong></div></div>{rankedShooters.map((shooter) => <div className="subcard" key={shooter.localId}><strong>{shooter.name} · {totalFor(shooter, targetResults)}/{sheetTotalTargets}</strong><p className="small muted">{postNumbers.map((post) => `${isCompak ? "Stand" : "Post"} ${post}: ${displayedPostScore(shooter, post - 1, targetResults)}/${expectedTargetsPerPost[post - 1]}`).join(" · ")}</p></div>)}<p><strong>{reviewCoverage.complete ? "Complete" : `Incomplete — ${reviewCoverage.unscored} targets remain unscored`}</strong></p>{reviewCoverage.unscored > 0 && <label className="checkboxRow"><input type="checkbox" checked={allowIncompleteFinalization} onChange={(event) => setAllowIncompleteFinalization(event.target.checked)} /> Finalize with incomplete targets</label>}{finalizationBlockReason && <div className="error">{finalizationBlockReason}</div>}<div className="btns"><button type="button" onClick={finalizeCompetition} disabled={saving || Boolean(finalizationBlockReason) || (reviewCoverage.unscored > 0 && !allowIncompleteFinalization)}>{saving ? "Finalizing..." : "Finalize result"}</button><button type="button" className="secondary" onClick={() => setShowFinalizationReview(false)} disabled={saving}>Cancel</button></div></section></div>}
      </div>
    </main>
  );
}
