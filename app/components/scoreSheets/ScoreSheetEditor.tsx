...
export default function ScoreSheetEditor({ kind }: { kind: "training" | "competition" }) {
  ...
  useEffect(() => {
    ...
    function handleOnline() {
      setIsOnline(true);
      if (hasUnsyncedLocalDraft && !saving && !syncBlockedByRecovery(Boolean(recoveryPrompt), recoveryAutosavePaused)) {
        setLocalSaveStatus("saved_local");
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hasUnsyncedLocalDraft, saving, recoveryPrompt, recoveryAutosavePaused]);
            ...
            <div className="liveScoringPanel">
              <div className="liveContextHeader">
                ...
                <div className={`syncStatusPill syncStatus-${localSaveStatus}`}>
                  ...
                  {hasUnsyncedLocalDraft && isOnline && localSaveStatus !== "syncing" && !syncBlockedByRecovery(Boolean(recoveryPrompt), recoveryAutosavePaused) && (
                    <button
                      type="button"
                      className="secondary smallButton"
                      onClick={() => save({ navigate: false, reload: false })}
                      disabled={saving}
                    >
                      {syncActionLabel(localSaveStatus)}
                    </button>
                  )}
                </div>
                <button
