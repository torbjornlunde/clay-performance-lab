...
const TYPES: Array<{ value: ProgrammePresentationType; label: string }> = [{ value: "single", label: "Single" }, { value: "report_pair", label: "Report pair" }, { value: "simultaneous_pair", label: "Simultaneous pair" }, { value: "unknown", label: "Unknown / legacy" }];
...
export default function ProgrammeEditor({ programme, disabled, onChange, onReset }: { programme: ScoreSheetProgramme; disabled: boolean; onChange: (value: ScoreSheetProgramme) => void; onReset: () => void }) {
  ...
  return <details className="subcard scoreSheetProgramme">
    ...
    {programme.areas.map((area, areaIndex) => <section className="programmeArea" key={area.areaNumber}>
      <div className="sectionHeader compactSectionHeader"><strong>Area {area.areaNumber}</strong><div className="btns"><span className="small muted">{programmeAreaTargetCount(area)} targets</span><button type="button" className="secondary smallButton" disabled={disabled || programme.areas.length === 1} onClick={() => update((draft) => { draft.areas.splice(areaIndex, 1); draft.areas.forEach((item, index) => { item.areaNumber = index + 1; }); })}>Remove area</button></div></div>
      {area.presentations.map((presentation, presentationIndex) => <div className="programmePresentation" key={presentation.id}>
        <label>Presentation<select value={presentation.type} disabled={disabled} onChange={(event) => update((draft) => { const item = draft.areas[areaIndex].presentations[presentationIndex]; item.type = event.target.value as ProgrammePresentationType; if (item.type === "single" || item.type === "unknown") item.secondMachine = null; })}>{TYPES.map((type) => <option value={type.value} key={type.value}>{type.label}</option>)}</select></label>
