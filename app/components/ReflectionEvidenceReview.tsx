...
export function ReflectionEvidenceReview({ sessionId, note, canInterpret = true }: { sessionId: string; note: { id: string; body: string; updated_at: string } | null; canInterpret?: boolean }) {
  const [items, setItems] = useState<Row[]>([]); const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  async function load() { const { data, error } = await supabase.from("private_reflection_evidence").select("id,category,normalized_value,label,evidence_basis,confidence,reference,review_status,source_note_updated_at").eq("session_id", sessionId).order("created_at", { ascending: false }); if (error) { setMessage("Evidence could not be loaded. Check your connection and try again."); return false; } setItems((data || []) as Row[]); return true; }
  useEffect(() => { void load(); }, [sessionId]);
  async function interpret() {
    if (!note?.body.trim() || !canInterpret) { setMessage("Save the reflection and wait for sync before interpreting it."); return; }
  ...
  return <div className="reflectionEvidenceReview" aria-labelledby="reflection-evidence-heading">
    <div className="reflectionEvidenceHeader"><div><h3 id="reflection-evidence-heading">Review coaching evidence</h3><p className="small muted">AI can suggest structure only. Nothing is used until you accept it.</p></div><button type="button" className="button secondary smallButton" disabled={busy || !note?.body.trim() || !canInterpret} onClick={() => void interpret()}>{busy ? "Working..." : "Interpret reflection"}</button></div>
    {!canInterpret && note && <p className="small warningInline">Save the visible reflection and wait for sync before interpreting it.</p>}
    ...
    {visible.map((item) => { const stale = !note || item.source_note_updated_at !== note.updated_at; return <article className="reflectionEvidenceItem" key={item.id}>
      ...
      <div className="reflectionEvidenceFields"><label>Value<input maxLength={80} value={item.normalized_value} disabled={item.review_status === "accepted"} onChange={(e) => edit(item.id, "normalized_value", e.target.value)} /></label><label>Post/course reference<input maxLength={20} pattern="(?:[Pp]ost|[Ss]tand|[Cc]ourse) [1-9][0-9]?" placeholder="Post 3" value={item.reference || ""} disabled={item.review_status === "accepted"} onChange={(e) => edit(item.id, "reference", e.target.value)} /></label></div>
