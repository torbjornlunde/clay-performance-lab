...
import { courseReviewForCourse, courseReviewSourceLabel, resolveCourseReviewSourceState, type CourseReviewLoadState } from "@/lib/scorecards/courseScorecardReview";
...
export default function ScorecardEvidenceSection({ sessionId, userId, courseCount }: { sessionId: string; userId: string; courseCount: number }) {
  ...
  const [reviewState, setReviewState] = useState<CourseReviewLoadState<CourseReview>>({ status: "loading" });
  ...
  async function load() {
    ...
    if (reviewResult.error) setReviewState({ status: "error", message: reviewResult.error.message });
    else setReviewState({ status: "ready", reviews: (reviewResult.data || []) as CourseReview[] });
  ...
  return <section className="card scorecardEvidenceSection" aria-labelledby="scorecard-evidence-title">
    ...
    {reviewState.status === "loading" && <p className="small muted">Loading course reviews…</p>}
    {reviewState.status === "error" && <div><p className="small" role="alert">Course reviews could not be loaded.</p><button type="button" className="button secondary smallButton" onClick={() => void load()}>Retry</button></div>}
    {reviewState.status === "ready" && reviewState.reviews.length > 0 && <div><strong>Course review coverage</strong><ul>{courseOptions.map((number)=>{const review=courseReviewForCourse(reviewState,number);const source=review?items.find(item=>item.id===review.evidence_id):null;const sourceLabel=review?courseReviewSourceLabel(resolveCourseReviewSourceState(review,source)):null;return <li key={number}>Course {number} · {!review?"Not reviewed":`${review.reviewed_score==null?"Score unknown":`${review.reviewed_score}/25`} · ${review.reviewed_unknowns?"Partial detail":"Complete"}${sourceLabel?` · ${sourceLabel}`:""}`}</li>;})}</ul></div>}
