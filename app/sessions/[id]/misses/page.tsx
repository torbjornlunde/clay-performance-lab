...
export default function MissesPage() {
  ...
  if (loading)
    return (
      <main>
        <div className="card">Loading misses...</div>
      </main>
    );
  if (!session)
    return (
  ...
  return (
    <main>
      <div className="card">
        ...
        <span className="pill">
          All misses <strong>{misses.length}</strong>
        </span>
        <div className="btns">
          ...
          misses.map((miss) => (
            <div className="card missReviewCard" key={miss.id}>
              <div className="missReviewHeader">
                <div>
                  ...
                  <div className="small muted">
                    {new Date(miss.created_at).toLocaleString()}
                  </div>
                </div>
              ...
              <div className="missReviewGrid">
                {isSporttrap ? (
                  <>
                    <DetailRow label="Series">
                      {value(miss.course_number)}
                    </DetailRow>
                    <DetailRow label="Stand">{value(miss.plate)}</DetailRow>
                    <DetailRow label="Sequence / presentation">
                      {value(normalizeLeirduestiLabel(miss.target_type))}
                    </DetailRow>
                    <DetailRow label="Target label">
                      {value(miss.target_label)}
                    </DetailRow>
                  </>
                ) : isLeirduesti ? (
                  <>
                    ...
                    <DetailRow label="Pair / sequence">
                      {value(miss.target_number)}
                    </DetailRow>
                  </>
                ) : (
                  <>
                    ...
                    <DetailRow label="Target/machine label">
                      {value(miss.target_label)}
                    </DetailRow>
                    <DetailRow label="Presentation/target type">
                      {value(normalizeLeirduestiLabel(miss.target_type))}
                    </DetailRow>
                  </>
                )}
                <DetailRow label="Base presentation">
                  {value(
                    normalizePresentation(
                      miss.base_presentation || miss.target_type,
                    ),
                  )}
                </DetailRow>
                ...
                <DetailRow label="Missed target">
                  {shortMissedTarget(miss.missed_target)}
                </DetailRow>
                <DetailRow label="Where miss">
                  {value(
                    miss.where_miss ||
                ...
                {miss.comment && (
                  <DetailRow label="Comment">{miss.comment}</DetailRow>
                )}
