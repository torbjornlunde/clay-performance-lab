...
export function PostTargetEditor({ session, courseRows }: Props) {
  ...
  useEffect(() => { let cancelled = false; const sessionId = session.id; const postNumber = current; setPendingPhoto(null); setReview(null); setError(""); void getPendingPostSignPhoto(sessionId, postNumber).then((item) => { if (cancelled || sessionIdRef.current !== sessionId || currentPostRef.current !== postNumber) return; setPendingPhoto(item); setReview(item?.analysis || null); }); return () => { cancelled = true; }; }, [session.id, current]);
  ...
  async function retryWaitingOnce() { const sessionId = sessionIdRef.current; const postNumber = currentPostRef.current; const item = await getPendingPostSignPhoto(sessionId, postNumber); if (item?.status === "waiting_for_connection") void analyze(item); }
  ...
  function clearCurrentPost() { const post = posts[current - 1]; if (postHasMeaningfulData(post) && !window.confirm(`Clear ${unit.toLowerCase()} will remove presentations, target descriptions, instructions and extracted source text. Continue?`)) return; mutate(posts.map((p, i) => i === current - 1 ? normalizePost(current, [], "", "") : p)); }
  ...
  async function analyze(item = pendingPhoto) { if (!item) return; const sessionId = item.sessionId; const postNumber = item.postNumber; const isStillCurrent = () => sessionIdRef.current === sessionId && currentPostRef.current === postNumber; if (!isStillCurrent()) return; if (!navigator.onLine) { const updated = await updatePendingPostSignPhoto(sessionId, postNumber, { status: "waiting_for_connection", lastError: "Waiting for connection" }); if (isStillCurrent()) setPendingPhoto(updated); return; } setError(""); const analyzing = await updatePendingPostSignPhoto(sessionId, postNumber, { status: "analyzing", lastError: undefined }); if (isStillCurrent()) setPendingPhoto(analyzing); const form = new FormData(); form.append("postNumber", String(postNumber)); form.append("image", item.image, "post-sign.jpg"); try { const { data: auth } = await supabase.auth.getSession(); const response = await fetch(`/api/sessions/${sessionId}/post-sign/analyze`, { method: "POST", headers: auth.session?.access_token ? { Authorization: `Bearer ${auth.session.access_token}` } : undefined, body: form }); const json = await response.json().catch(() => ({})); if (!response.ok) { const updated = await updatePendingPostSignPhoto(sessionId, postNumber, { status: "analysis_failed", lastError: json.error || "Analysis failed" }); if (isStillCurrent()) { setPendingPhoto(updated); setError(json.error || "Analysis failed"); } return; } const updated = await updatePendingPostSignPhoto(sessionId, postNumber, { status: "ready_for_review", analysis: json.result }); if (isStillCurrent()) { setPendingPhoto(updated); setReview(json.result); } } catch { const updated = await updatePendingPostSignPhoto(sessionId, postNumber, { status: "analysis_failed", lastError: "Network error. Retry analysis when connected." }); if (isStillCurrent()) { setPendingPhoto(updated); setError("Network error. Retry analysis when connected."); } } }
