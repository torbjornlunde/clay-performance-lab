        ...
        return <article className="statListItem trainingScoreSheetArchiveItem" key={`${result.score_sheet_id}:${result.shooter_id}`}>
          <div>
            ...
            <div className="btns archiveActions">
              {result.claimed_session_id ? <><span className="badge badgeBlue">Added to Results</span><Link className="button secondary smallButton" href={`/sessions/${result.claimed_session_id}`}>Open result</Link></> :
