...
export default function EquipmentPage() {
  ...
  async function saveWeapon(event: React.FormEvent) {
    ...
    const payload = { user_id: userId, display_name: weaponForm.display_name.trim(), manufacturer: clean(manufacturer), model: clean(model), weapon_type: weaponForm.weapon_type, gauge: chosenGauge(weaponForm.gauge, weaponForm.customGauge), is_default: weaponForm.is_default, initial_shot_count: initialCount, choke_configuration_type: weaponForm.choke_configuration_type };
  ...
  function editChoke(weaponId: string, choke: Choke) {
    ...
    window.setTimeout(() => document.getElementById("choke-editor")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  }
...
function SearchableCombobox({ label, value, options, onChange, placeholder }: { label: string; value: string; options: string[]; onChange: (value: string) => void; placeholder?: string }) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const wrapRef = useRef<HTMLLabelElement>(null);
  useEffect(() => {
    function close(event: MouseEvent) { if (wrapRef.current && !wrapRef.current.contains(event.target as Node)) setOpen(false); }
  ...
  const filtered = options.filter((option) => option.toLowerCase().includes((query || value).toLowerCase()));
  return <label className="comboField" ref={wrapRef}>{label}<span className="comboControl"><input value={value} placeholder={placeholder} onFocus={() => setOpen(true)} onClick={() => setOpen(true)} onChange={(event) => { onChange(event.target.value); setQuery(event.target.value); setOpen(true); }} /><button type="button" aria-label={`Show ${label} options`} onClick={() => { setQuery(""); setOpen(!open); }}>⌄</button></span>{open && <div className="comboPanel"><input className="comboSearch" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search or type custom" autoFocus /><button type="button" className="comboOption" onClick={() => { onChange(value); setOpen(false); }}>Use “{value || "custom value"}”</button>{filtered.map((option) => <button type="button" className="comboOption" key={option} onClick={() => { onChange(option === OTHER_CUSTOM ? "" : option); setOpen(false); setQuery(""); }}>{option}</button>)}</div>}</label>;
}
...
function ChokeSelectionSheet({ title, chokes, selectedId, onSelect, onClose }: { title: string; chokes: Choke[]; selectedId: string; onSelect: (chokeId: string) => void; onClose: () => void }) {
  useEffect(() => { function onKey(event: KeyboardEvent) { if (event.key === "Escape") onClose(); } document.addEventListener("keydown", onKey); return () => document.removeEventListener("keydown", onKey); }, [onClose]);
  return <div className="sheetBackdrop" role="presentation" onMouseDown={onClose}><div className="chokeSheet" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}><div className="sheetHeader"><h3>{title}</h3><button type="button" className="secondary smallButton" onClick={onClose}>Close</button></div><button type="button" className="chokeOption" onClick={() => onSelect("")}><strong>Not set</strong></button>{sortChokesForSelection(chokes).map((choke) => <button type="button" className={`chokeOption ${selectedId === choke.id ? "selected" : ""}`} key={choke.id} onClick={() => onSelect(choke.id)}><strong>{chokePrimaryLabel(choke)}</strong>{chokeSecondaryLabel(choke) && <small>{chokeSecondaryLabel(choke)}</small>}</button>)}</div></div>;
}
