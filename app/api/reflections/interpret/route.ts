...
import { reflectionEvidenceJsonSchema, reflectionSupportsReference, validateReflectionEvidenceOutput } from "@/lib/ai/reflectionEvidence";
...
export async function handleReflectionInterpret(request: Request, deps: Deps = {}) {
  ...
  const usage = await supabase.from("feature_usage_events").select("id", { count: "exact", head: true }).eq("user_id", userId).eq("feature_key", "ai.reflection_interpretation").eq("event_type", "used");
  if (profile.error) return NextResponse.json({ error: "AI access could not be verified." }, { status: 403 });
  if (usage.error) return NextResponse.json({ error: "AI preview usage could not be verified." }, { status: 503 });
  try { requirePaidCostAccess("ai.reflection_interpretation", createEntitlementUserContext({ userId, accessProfile: profile.data || null, entitlement: entitlement.error ? null : entitlement.data || null, billingMode: getBillingMode(deps.env || process.env), trialUsage: { "ai.reflection_interpretation": usage.count || 0 } })); } catch (error) { if (error instanceof FeatureAccessError) return NextResponse.json({ error: error.message }, { status: error.status }); throw error; }
  const existing = await supabase.from("private_reflection_evidence").select("*").eq("source_note_id", note.id).eq("source_note_updated_at", note.updated_at).order("created_at", { ascending: false });
  if (existing.error) return NextResponse.json({ error: "Existing evidence could not be checked. Nothing was changed." }, { status: 503 });
  if (existing.data?.length) return NextResponse.json({ items: existing.data, reused: true });
  ...
  try {
    ...
    let items; try { items = validateReflectionEvidenceOutput(parsed); if (items.some((item) => !reflectionSupportsReference(reflection, item.reference))) throw new Error("Unsupported reference"); } catch { return NextResponse.json({ error: "Interpretation returned unsupported evidence. Nothing was saved." }, { status: 502 }); }
