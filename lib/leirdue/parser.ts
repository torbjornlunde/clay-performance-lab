...
function emptyDebug(): LeirdueSearchDebug {
  return {
    ...
    autoContinuedBecausePendingQueue: false,
    autoStopReason: null,
    selectedYearWorkRemaining: false,
...
function detectDetailedListType(text: string) {
  const normalized = normalizeText(text);
  if (/(lagskyting|lagliste|\blag\b|team)/.test(normalized)) return "team/lagskyting list";
  if (/(finaleliste|\bfinale\b|shoot[-\s]?off|omskyting)/.test(normalized)) return "final list";
  if (/(cup sammenlagt|sammenlagt premiering|sammenlagt|flere stevner|multi-event|resultater med flere stevner)/.test(normalized)) return "summary/multi-event list";
  if (/(klassedelt|klassevis|klasse resultat|klasse\b)/.test(normalized)) return "class result list";
  if (directListScore(text) > 0 || /(resultatliste|resultater|etter bane)/.test(normalized)) return "individual result list";
  return "unknown list";
}

function shooterRowContext(lines: string[], shooterName: string) {
  const index = lines.findIndex((line) => pageContainsShooter(line, shooterName));
  if (index < 0) return { before: null, after: null };
  return {
    before: lines.slice(Math.max(0, index - 5), index).join(" | ") || null,
    after: lines.slice(index + 1, Math.min(lines.length, index + 6)).join(" | ") || null,
  };
}

function isNmLeirduestiContext(text: string) {
  const normalized = normalizeText(text);
  return normalized.includes("nm") && normalized.includes("leirduesti");
}
...
function updateCandidateDebugStats(debug: LeirdueSearchDebug, candidates: LeirdueCandidate[]) {
  ...
  for (const candidate of candidates) {
    ...
    const listTypeDetected = candidate.notes.match(/listTypeDetected=([^.;]+)/)?.[1] || null;
    const nmRowContextBefore = candidate.notes.match(/nmRowContextBefore=([^;.]*)/)?.[1] || null;
    const nmRowContextAfter = candidate.notes.match(/nmRowContextAfter=([^.]*)/)?.[1] || null;
...
function debugParseLeirdueHtml(params: { url: string; status: number | null; html: string; shooterName: string; year: number; selectedDisciplines: string[]; parserNote: string }): LeirdueDebugParseResult {
  ...
  const listTypeDetected = detectDetailedListType(`${listTitle}\n${parsed.scoreLine || ""}\n${rawSnippet || ""}`);
  const nmContext = isNmLeirduestiContext(`${listTitle}\n${eventTitle}\n${pageText.slice(0, 2000)}`) ? shooterRowContext(lines, shooterName) : { before: null, after: null };
  ...
  parserNotes.push(`listTypeDetected=${listTypeDetected}.`);
  if (nmContext.before || nmContext.after) parserNotes.push(`nmRowContextBefore=${nmContext.before || ""}; nmRowContextAfter=${nmContext.after || ""}.`);
...
export async function searchLeirdueCandidates(input: LeirdueSearchInput): Promise<LeirdueSearchResult> {
  ...
  const selectedYearWorkRemaining = debug.pendingListeIdQueueRemaining > 0 || debug.confirmedSelectedYearEventsRemaining > 0 || debug.likelySelectedYearEventsRemaining > 0;
  debug.selectedYearWorkRemaining = selectedYearWorkRemaining;
  debug.autoContinuedBecausePendingQueue = debug.pendingListeIdQueueRemaining > 0 && debug.importableCompleteCandidates < TARGET_COMPLETE_CANDIDATES;
  const likelySelectedYearWorkRemaining = selectedYearWorkRemaining;
  ...
  debug.safetyLimitReached = Boolean(debug.timedOut || debug.limitReached);
  debug.autoStopReason = canContinue
    ? null
    : debug.completeCandidatesFoundTotal >= TARGET_COMPLETE_CANDIDATES
      ? "targetReached"
      : onlyOldFallbackRemains
        ? "onlyOldFallbackRemains"
        : debug.timedOut
          ? "safetyTimeLimitReached"
          : !selectedYearWorkRemaining
