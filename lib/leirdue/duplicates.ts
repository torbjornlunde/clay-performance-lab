...
export function compareLeirdueDuplicate(candidate: LeirdueCandidate, row: LeirdueDuplicateSessionRow): LeirdueDuplicateMatch | null {
  ...
  const sameShooter = namesLikelyMatch(noteValue(row.notes, "shooter_name") || row.name, candidate.shooterName || candidate.name);
  ...
  if ((sameUrl || sameSourceIds) && sameShooter && sameOwnScore && sameTotalTargets) {
    return { id: row.id, exact: true, reason: sameUrl ? "Duplicate reason: same Leirdue source URL + shooter + score." : "Duplicate reason: same source stevne_id + liste_id + shooter + score." };
  }
