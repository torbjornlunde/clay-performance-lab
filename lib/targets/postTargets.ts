...
export function normalizeSharedPost(post_number: number, physicalTargets: PhysicalTarget[] = [], sharedPresentations: SharedPresentation[] = [], instructions = "", source_text = "", compatibilityWarnings: string[] = []): PostTargets {
  const sourceTargets = new Map(physicalTargets.map((target, index) => [target.id || `target-${index + 1}`, target]));
  const remappedOverrides = new Map<string, Record<string, TargetDescription>>();
  const normalizedShared = sharedPresentations.map((p, i) => {
    const presentationNumber = i + 1;
    return {
      ...
      legacy_override_keys: Array.from({ length: targetCountFor(p.presentation_type) }, (_, idx) => {
        const targetId = p.target_ids[idx] || "";
        const oldKey = p.legacy_override_keys ? (p.legacy_override_keys[idx] || "") : `${p.presentation_number || presentationNumber}:${idx + 1}`;
        const override = oldKey && targetId ? sourceTargets.get(targetId)?.legacy_overrides?.[oldKey] : undefined;
        if (!targetId || !override) return "";
        const newKey = `${presentationNumber}:${idx + 1}`;
        if (!remappedOverrides.has(targetId)) remappedOverrides.set(targetId, {});
        remappedOverrides.get(targetId)![newKey] = override;
        return newKey;
      }),
    };
  });
  ...
  physicalTargets.forEach((t, i) => {
    ...
    const legacy_overrides = Object.fromEntries(Object.entries(remappedOverrides.get(id) || {}).filter(([key]) => activeKeys.has(key)));
...
export function isSharedPresentationComplete(post: PostTargets, presentation: SharedPresentation | undefined) { if (!presentation) return false; const byId = new Map(post.physicalTargets.map((target) => [target.id, target])); return Array.from({ length: targetCountFor(presentation.presentation_type) }, (_, idx) => presentation.target_ids[idx] || "").every((targetId) => { const target = byId.get(targetId); return Boolean(targetId && target && normalizeTargetLabel(target.target_label)); }); }
export function duplicateSharedPresentation(post: PostTargets, index: number): PostTargets { const source = post.sharedPresentations[index]; if (!isSharedPresentationComplete(post, source)) return post; return normalizeSharedPost(post.post_number, post.physicalTargets, [...post.sharedPresentations, { presentation_number: post.sharedPresentations.length + 1, presentation_type: source!.presentation_type, target_ids: [...source!.target_ids], legacy_override_keys: [] }], post.instructions, post.source_text, post.compatibilityWarnings || []); }
export function copyPresentationToRemaining(post: PostTargets, index: number, expectedPresentationCount: number): PostTargets { const source = post.sharedPresentations[index]; if (!isSharedPresentationComplete(post, source)) return post; const next = [...post.sharedPresentations]; for (let i = index + 1; i < expectedPresentationCount; i++) { const existing = next[i]; const copy = { presentation_number: i + 1, presentation_type: source!.presentation_type, target_ids: [...source!.target_ids], legacy_override_keys: [] }; if (!existing) next[i] = copy; else if (!existing.target_ids.some(Boolean)) next[i] = copy; } return normalizeSharedPost(post.post_number, post.physicalTargets, next, post.instructions, post.source_text, post.compatibilityWarnings || []); }

export type RepeatedProgramme = { repeatCount: number; presentationType: PresentationType; targetIds: string[]; targetPositions: number; summary: string };
export function validateRepeatCount(expectedTargetPositions: number, presentationType: PresentationType) { const per = targetCountFor(presentationType); if (!Number.isFinite(expectedTargetPositions) || expectedTargetPositions < per) return { ok: false as const, message: `Expected target positions must be at least ${per}.` }; if (expectedTargetPositions % per !== 0) return { ok: false as const, message: `${expectedTargetPositions} target positions cannot be divided into ${per}-target presentations.` }; return { ok: true as const, repeatCount: expectedTargetPositions / per }; }
export function applyRepeatedProgramme(post: PostTargets, repeatCount: number, presentationType: PresentationType, targetIds: string[]) { const count = targetCountFor(presentationType); const ids = Array.from({ length: count }, (_, i) => targetIds[i] || ""); const sharedPresentations = Array.from({ length: Math.max(0, Math.floor(repeatCount)) }, (_, i) => ({ presentation_number: i + 1, presentation_type: presentationType, target_ids: [...ids], legacy_override_keys: [] as string[] })); return normalizeSharedPost(post.post_number, post.physicalTargets, sharedPresentations, post.instructions, post.source_text, post.compatibilityWarnings || []); }
export function detectRepeatedProgramme(post: PostTargets): RepeatedProgramme | null { const presentations = post.sharedPresentations || []; if (!presentations.length) return null; const byId = new Map(post.physicalTargets.map((t) => [t.id, t])); const first = presentations[0]; const count = targetCountFor(first.presentation_type); const ids = Array.from({ length: count }, (_, i) => first.target_ids[i] || ""); if (ids.some((id) => !id || !byId.has(id))) return null; for (const p of presentations) { if (p.presentation_type !== first.presentation_type || targetCountFor(p.presentation_type) !== count) return null; const current = Array.from({ length: count }, (_, i) => p.target_ids[i] || ""); if (current.some((id, i) => id !== ids[i] || !byId.has(id))) return null; if ((p.legacy_override_keys || []).some(Boolean)) return null; }
  if (post.physicalTargets.some((t) => t.legacy_conflict || Object.keys(t.legacy_overrides || {}).length || (t.legacy_warnings || []).length)) return null;
  const labels = ids.map((id) => byId.get(id)?.target_label || "Unassigned"); const connector = first.presentation_type === "simultaneous_pair" ? " + " : " → "; return { repeatCount: presentations.length, presentationType: first.presentation_type, targetIds: ids, targetPositions: presentations.length * count, summary: `${presentations.length} × ${labels.join(connector)} · ${presentationLabels[first.presentation_type]}` }; }
export function shouldConfirmPhysicalTargetDeletion(target: PhysicalTarget | undefined) { return Boolean(target && (target.legacy_conflict || Object.keys(target.legacy_overrides || {}).length || (target.legacy_warnings || []).length || target.notes?.trim() || [target.target_type,target.direction,target.angle,target.speed,target.distance,target.difficulty].some((v)=>v && v !== "Unknown"))); }
export function affectedTargetReferences(post: PostTargets, targetId: string) { const presentationNumbers: number[] = []; let referenceCount = 0; post.sharedPresentations.forEach((p) => { const count = p.target_ids.filter((id) => id === targetId).length; if (count) { presentationNumbers.push(p.presentation_number); referenceCount += count; } }); return { referenceCount, presentationNumbers }; }
export function deletePhysicalTargetAndClearReferences(post: PostTargets, targetId: string) { const target = post.physicalTargets.find((t) => t.id === targetId); const warningText = new Set([...(target?.legacy_warnings || [])]); const physicalTargets = post.physicalTargets.filter((t) => t.id !== targetId); const sharedPresentations = post.sharedPresentations.map((p) => ({ ...p, target_ids: p.target_ids.map((id) => id === targetId ? "" : id), legacy_override_keys: (p.legacy_override_keys || []).map((key, i) => p.target_ids[i] === targetId ? "" : key) })); const compatibilityWarnings = (post.compatibilityWarnings || []).filter((w) => !warningText.has(w) && !(target?.target_label && w.includes(`Target ${normalizeTargetLabel(target.target_label)}`))); return normalizeSharedPost(post.post_number, physicalTargets, sharedPresentations, post.instructions, post.source_text, compatibilityWarnings); }
export function removeSharedPresentation(post: PostTargets, index: number) { return normalizeSharedPost(post.post_number, post.physicalTargets, post.sharedPresentations.filter((_, i) => i !== index), post.instructions, post.source_text, post.compatibilityWarnings || []); }
