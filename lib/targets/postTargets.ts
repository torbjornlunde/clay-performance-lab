...
export type TargetDescription = { target_type: string; direction: string; speed: string; distance: string; difficulty: string; notes: string };
...
export function blankTarget(target_position: number, position_in_presentation: number): PostTarget { return { target_position, position_in_presentation, target_type: "Unknown", direction: "Unknown", speed: "Unknown", distance: "Unknown", difficulty: "Unknown", notes: "" }; }
...
export function normalizePost(post_number: number, presentations: Presentation[]): PostTargets { let position = 1; return { post_number, presentations: presentations.map((p, i) => ({ presentation_number: i + 1, presentation_type: p.presentation_type, targets: Array.from({ length: targetCountFor(p.presentation_type) }, (_, idx) => ({ ...blankTarget(position + idx, idx + 1), ...(p.targets[idx] || {}), target_position: position + idx, position_in_presentation: idx + 1 })) })).map((p) => { position += p.targets.length; return p; }) }; }
...
export function rowsFromPosts(sessionId: string, posts: PostTargets[]) { return posts.flatMap((post) => post.presentations.flatMap((p) => p.targets.map((t) => ({ session_id: sessionId, post_number: post.post_number, target_position: t.target_position, presentation_number: p.presentation_number, presentation_type: p.presentation_type, position_in_presentation: t.position_in_presentation, target_type: t.target_type, direction: t.direction, speed: t.speed, distance: t.distance, difficulty: t.difficulty, notes: t.notes.trim() || null, updated_at: new Date().toISOString() })))); }
