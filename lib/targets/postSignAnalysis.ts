...
export function validatePostSignAnalysis(value: unknown): PostSignAnalysisResult {
  ...
  result.presentations = raw.presentations.flatMap((item: any, index: number): PostSignPresentation[] => {
    ...
    const count = item?.repetitionCount === undefined ? 1 : item.repetitionCount;
    if (!Number.isInteger(count) || count < 1 || count > 25) throw new Error(`Presentation ${index + 1} has an invalid repetition count.`);
    const presentation: PostSignPresentation = { presentationNumber: 0, presentationType: structuralKind === "pair" && typeEvidence === "user_convention_required" ? "unknown" : type, structuralKind, targetLabels: labels, sourceText: typeof item?.sourceText === "string" ? item.sourceText.slice(0, 500) : "", sourceNotation: typeof item?.sourceNotation === "string" ? item.sourceNotation.slice(0, 80) : "", notationKind, typeEvidence, confidence: confidence.has(item?.confidence) ? item.confidence : "low", warnings: Array.isArray(item?.warnings) ? item.warnings.filter((x: unknown) => typeof x === "string").map((x: string) => x.slice(0, 300)) : [] };
    return Array.from({ length: count }, () => ({ ...presentation }));
  });
  if (result.presentations.length > 50) throw new Error("Too many presentations on one sign.");
  result.presentations.forEach((presentation, index) => { presentation.presentationNumber = index + 1; });
  return result;
}
...
export const postSignAnalysisJsonSchema = { type: "object", additionalProperties: false, required: ["detectedPostNumbers","rawText","instructions","confidence","warnings","presentations"], properties: { detectedPostNumbers: { type: "array", items: { type: "integer" } }, rawText: { type: "string" }, instructions: { type: "string" }, confidence: { type: "string", enum: ["high","medium","low"] }, warnings: { type: "array", items: { type: "string" } }, presentations: { type: "array", items: { type: "object", additionalProperties: false, required: ["presentationNumber","repetitionCount","presentationType","structuralKind","targetLabels","sourceText","sourceNotation","notationKind","typeEvidence","confidence","warnings"], properties: { presentationNumber: { type: "integer" }, repetitionCount: { type: "integer" }, presentationType: { type: "string", enum: ["single","report_pair","simultaneous_pair","other_pair","unknown"] }, structuralKind: { type: "string", enum: ["single","pair"] }, targetLabels: { type: "array", items: { type: "string" } }, sourceText: { type: "string" }, sourceNotation: { type: "string", maxLength: 80 }, notationKind: { type: "string", enum: ["single","plus","joined","arrow","explicit_report","explicit_simultaneous","other"] }, typeEvidence: { type: "string", enum: ["explicit_heading","explicit_wording","user_convention_required","uncertain"] }, confidence: { type: "string", enum: ["high","medium","low"] }, warnings: { type: "array", items: { type: "string" } } } } } } } as const;
