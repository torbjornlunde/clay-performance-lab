...
function normalizeDiscipline(value?: string | null) { return String(value || "").trim().toLowerCase().replace(/\s+/g, " "); }
...
function setupFor(miss: AnalysisMiss, setup: PostTargetAnalysisRow[]) {
  return setup.find(row => Number(row.post_number) === Number(miss.course_number) && Number(row.target_position) === Number(miss.target_position));
}
...
export function buildDeterministicSessionAnalysis(input: {
  ...
  const score = input.scorecardImport?.reviewed_hits ?? input.session.own_score ?? Math.max(0, Number(input.session.total_targets || 0) - input.misses.length);
  const totalTargets = input.scorecardImport?.reviewed_total_targets ?? input.session.total_targets ?? (score + input.misses.length);
  const missTotal = input.scorecardImport?.reviewed_misses ?? input.misses.length;
  ...
  const byPost = count(input.misses.map(m => m.course_number));
  const topPost = topSupported(byPost, missTotal, 2);
  if (topPost) findings.push(`${topPost[1]} of ${missTotal} misses came on post ${topPost[0]}.`);
  if (byPost.length >= 2 && byPost[0][1] + byPost[1][1] >= Math.max(3, Math.ceil(missTotal * ANALYSIS_THRESHOLDS.concentrationShare))) {
    findings.push(`Most misses came on posts ${byPost[0][0]} and ${byPost[1][0]} (${byPost[0][1] + byPost[1][1]} of ${missTotal}).`);
  }
  const thirds = { early: 0, middle: 0, late: 0 };
  for (const miss of input.misses) {
    const order = Number(miss.target_position || miss.target_number || 0);
    const section = order <= totalTargets / 3 ? "early" : order <= (totalTargets * 2) / 3 ? "middle" : "late";
    thirds[section as keyof typeof thirds]++;
  }
  const topThird = Object.entries(thirds).sort((a,b)=>b[1]-a[1])[0];
  if (topThird && topThird[1] >= Math.max(3, Math.ceil(missTotal * ANALYSIS_THRESHOLDS.dominantShare))) findings.push(`Misses were concentrated ${topThird[0]} in the round (${topThird[1]} of ${missTotal}).`);

  const mapped = input.misses.map(m => ({ miss:m, setup: setupFor(m, postTargets) }));
  const mappedCount = mapped.filter(x => x.setup).length;
  if (input.misses.length && mappedCount < input.misses.length) missingData.push("Target descriptions are missing for some imported misses, so target type, direction and pair-position patterns are limited.");
  const posEntries = count(mapped.map(x => x.setup?.position_in_presentation === 1 ? "first target" : x.setup?.position_in_presentation === 2 ? "second target" : null));
  const topPos = topSupported(posEntries, mappedCount);
  if (topPos) findings.push(`${topPos[1]} misses were on the ${topPos[0]} of a presentation.`);
  const label = topSupported(count(mapped.map(x => key(x.setup?.target_label))), mappedCount);
  if (label) findings.push(`Target ${label[0]} was missed ${label[1]} times.`);
  const direction = topSupported(count(mapped.map(x => key(x.setup?.direction))), mappedCount);
  if (direction) findings.push(`${direction[1]} misses involved ${direction[0]} targets.`);
  const targetType = topSupported(count(mapped.map(x => key(x.setup?.target_type))), mappedCount);
  if (targetType) findings.push(`${targetType[1]} misses involved ${targetType[0]} targets.`);
  ...
  if (topPost) recommendations.push({ title: `Recreate post ${topPost[0]} first.`, evidence: `It contained ${topPost[1]} of the ${missTotal} misses.` });
  if (topPos) recommendations.push({ title: `Train the transition into the ${topPos[0]}.`, evidence: `${topPos[1]} misses occurred in that mapped presentation position.` });
  if (topThird?.[0] === "late" && topThird[1] >= 3) recommendations.push({ title: "Use a short reset routine before the final third.", evidence: `The miss count was highest late in the round (${topThird[1]} misses).` });
  if (!postTargets.length && input.misses.length) recommendations.push({ title: "Add target descriptions for the missed posts.", evidence: "Without setup rows, direction, speed and target-type repeats cannot be proven." });
  if (!recommendations.length && input.misses.length) recommendations.push({ title: "Repeat the most common missed presentations from this scorecard.", evidence: "The scorecard gives exact missed post and target positions even without manual reasons." });
  ...
  const history = (input.history || [])
    ...
    .slice(0, ANALYSIS_THRESHOLDS.recentHistoryLimit);
  function comparison(type: "Competition" | "Training") {
    const rows = history.filter(s => s.session_type === type);
  ...
  return {
    summary: { score, totalTargets, hitPercentage: currentPct, misses: missTotal },
    findings,
