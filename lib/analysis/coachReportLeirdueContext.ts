...
function conservativeEventListKey(row: LeirdueResultRow) { return [clean(row.event_id), clean(row.liste_id), normalizeLeirdueUrl(row.source_url), String(row.event_date || "").slice(0, 10), lower(row.event_title), lower(row.organizer), normalizeDisciplineGroup(row.discipline)].join("|"); }
