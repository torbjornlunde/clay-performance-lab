...
function cleanAnalysisLabel(value: string | number | null | undefined) {
...
function countBy(values: (string | number | null | undefined)[]) {
  ...
  for (const value of values) {
    const key = cleanAnalysisLabel(value);
...
export function analyzeMisses(misses: MissForAnalysis[]) {
  ...
  const pair = detailedMisses.filter((miss) =>
    `${miss.target_type ?? ""} ${miss.targetPosition ?? ""}`
      .toLowerCase()
      .match(/double|pair/),
  ).length;
  const first = detailedMisses.filter(
    (miss) => miss.targetPosition === "First target in pair",
  ).length;
  const second = detailedMisses.filter(
    (miss) => miss.targetPosition === "Second target in pair",
  ).length;
  ...
  if (!total) {
    ...
    if (pair >= Math.ceil(total * 0.5))
      interpretation.push(
        "Most misses are connected to pairs. Focus on plan, rhythm and transition.",
      );
    if (second >= Math.ceil(total * 0.35)) {
      interpretation.push(
        "Several misses are on the second target in pairs. This suggests pickup point, transition timing, or late first shot.",
      );
    }
    if (first >= Math.ceil(total * 0.35)) {
      interpretation.push(
        "Several misses are on the first target in pairs. This suggests setup, hold point, visual pickup, or target reading before the first shot.",
      );
    }
  ...
  if (topKey(byTargetPosition) === "Second target in pair")
    recommendation.push(
      "Run pair drills with fixed first break point and pre-decided second pickup point.",
    );
  if (topKey(byTargetPosition) === "First target in pair")
    recommendation.push(
      "Rehearse the setup, hold point, visual pickup and first break point before calling.",
    );
