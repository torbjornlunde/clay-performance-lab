...
export async function renderCropBlob(image: Blob, crop: NormalizedCrop): Promise<Blob> {
  ...
  try {
    const c = clampCrop(crop);
    const sx = Math.round(c.x * bmp.width), sy = Math.round(c.y * bmp.height), sw = Math.max(1, Math.round(c.width * bmp.width)), sh = Math.max(1, Math.round(c.height * bmp.height));
