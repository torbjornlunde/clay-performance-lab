...
export function countUnscoredTargets(targetResults: TargetResultMap, shooterIds: string[], setup: ExpectedTargetSetup) {
  return normalizeExpectedTargetsByPost(setup).reduce((remaining, expectedTargets, postIndex) => {
    const scored = shooterIds.reduce((sum, shooterId) => sum + targetStatsForPost(targetResults, shooterId, postIndex + 1).scored, 0);
    return remaining + Math.max(shooterIds.length * expectedTargets - scored, 0);
  }, 0);
}

export function scoreSheetCompletionStatus(targetResults: TargetResultMap, shooterIds: string[], setup: ExpectedTargetSetup) {
  const expectedEntries = shooterIds.length * getTotalExpectedTargets(setup);
  const remainingEntries = countUnscoredTargets(targetResults, shooterIds, setup);
  return { expectedEntries, scoredEntries: expectedEntries - remainingEntries, remainingEntries, complete: expectedEntries > 0 && remainingEntries === 0 };
}

export type TargetResultWrite = { score_sheet_id: string; shooter_id: string; post_number: number; target_number: number; result: TargetResultValue };
export function deduplicateTargetResultWrites(rows: readonly TargetResultWrite[]) {
  const byIdentity = new Map<string, TargetResultWrite>();
  rows.forEach((row) => byIdentity.set(targetResultUpsertKey(row.score_sheet_id, row.shooter_id, row.post_number, row.target_number), row));
