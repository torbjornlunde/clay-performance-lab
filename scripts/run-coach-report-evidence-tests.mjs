...
const privateNotesBySession = {
  ...
  t1: [{ id: 'n3', updated_at: '2026-07-01T10:00:00Z', note_scope: 'session', body: 'training secret', context_tags: [] }],
};
...
assert.deepEqual(evidence.selfReportedContext.map((item) => [item.tag, item.sessionCount]), [['tired', 2], ['rabbit', 1]], 'explicit tags remain self-reported context with distinct-session counts');
...
assert(evidence.confidence.reasons.some((reason) => /sessions.*score data|target details/.test(reason)), 'confidence exposes deterministic reasons');
