...
let stillBad=a.deriveCurrentPostReconciliation({currentCells:[mk(1,1,'hit','high',null,null,true),mk(1,2,'hit','high',null,null,true)],detectedPostScore:1,detectedPostScoreConfidence:'high',expectedTargetCount:2,originalStatus:'conflict',originalWarning:'bad'}); assert.equal(stillBad.reconciliationStatus,'needs_review','complete user-reviewed cells can override a conflicting AI-detected total'); assert.match(stillBad.reconciliationWarning,/reviewed cells will be used/i,'AI total disagreement is explained as reference, not a blocker');
...
let completeDisagree=a.deriveCurrentPostReconciliation({currentCells:[mk(1,1,'hit','high',null,null,true),mk(1,2,'miss','high',null,null,true)],detectedPostScore:2,detectedPostScoreConfidence:'high',expectedTargetCount:2,originalStatus:'conflict',originalWarning:'bad'}); assert.equal(completeDisagree.reconciliationStatus,'needs_review','complete reviewed grid may proceed even when it disagrees with AI row total');
let explicitBefore=aiHitUserMiss[0]; let afterRec=a.deriveCurrentPostReconciliation({currentCells:aiHitUserMiss,detectedPostScore:1,detectedPostScoreConfidence:'high',expectedTargetCount:1,originalStatus:'conflict',originalWarning:'bad'}); assert.equal(aiHitUserMiss[0].result,explicitBefore.result,'reconciliation audit does not overwrite explicit reviewed values'); assert.equal(afterRec.reviewedScore,0,'reconciliation audit reports reviewed score from explicit cells');
