...
let unconfirmedConflict=a.confirmCurrentPostReview({grid:[mk(1,1,'hit','high'),mk(1,2,'hit','high')],currentPost:1,postCount:1,reviewedPostNumbers:[],postStatuses:{1:'conflict'}}); assert.equal(unconfirmedConflict.ok,false,'unreviewed high-confidence AI-total conflict blocks post completion'); let confirmedMismatch=a.confirmCurrentPostReview({grid:[mk(1,1,'hit','high'),mk(1,2,'hit','high')],currentPost:1,postCount:1,reviewedPostNumbers:[],postStatuses:{1:'needs_review'}}); assert.equal(confirmedMismatch.ok,true,'explicit post confirmation allows complete reviewed mismatch warning to proceed');
...
assert.match(globalCssForReview, /@media \(max-width: 430px\) \{[\s\S]*\.scorecardGrid\s*\{[\s\S]*grid-template-columns:\s*1fr/, 'narrow scorecard review uses single-column cards to avoid control collisions');
assert.match(globalCssForReview, /\.scorecardCellChoices\s*\{[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,1fr\)\)/, 'scorecard review choices remain visible Hit/Miss/? controls');
