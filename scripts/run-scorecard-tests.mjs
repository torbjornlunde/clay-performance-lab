...
assert.equal(profiles.isScorecardImportDiscipline('English Sporting'), true, 'English Sporting remains allowed in scorecard import');
assert.equal(profiles.isScorecardImportDiscipline('FITASC Sporting'), true, 'FITASC Sporting is allowed in scorecard import');
const fitascProfile = profiles.scorecardDisciplineProfile('FITASC Sporting');
assert.deepEqual(fitascProfile, {key:'post_based',unitLabel:'Stand',reviewLabel:'Stand'}, 'FITASC Sporting uses the generic stand-based review model');
assert.equal(profiles.isPostBasedScorecardImportDiscipline('FITASC Sporting'), true, 'FITASC Sporting can use post-based structure discovery');
let fitascNoSetup = profiles.resolveDisciplineScorecardSetup({discipline:'FITASC Sporting',postCount:null,courseCount:null,targetsPerPost:null,totalTargets:null,targetDefinitions:[]});
assert.equal(fitascNoSetup.ok,false, 'FITASC Sporting without setup enters discovery instead of inventing a fixed structure');
const fitascVariableCounts=[25,24,26,25];
const fitascDiscovered=a.normalizeScorecardAnalysis({...discoveryAnalysis,rawText:'synthetic FITASC card',shooterRows:[{...discoveryAnalysis.shooterRows[0],posts:fitascVariableCounts.map((count,idx)=>({postNumber:idx+1,expectedTargets:count,detectedPostScore:count-2,detectedPostScoreConfidence:'high',detectedPostScoreRawText:String(count-2),targets:Array.from({length:count},(_,i)=>({targetNumber:i+1,cellState:'active',result:i<count-2?'hit':'miss',rawMark:i<count-2?'/':'0',observedMarkCategory:i<count-2?'diagonal_stroke':'zero',confidence:'high',warning:null}))}))}]},{totalTargets:100,allowStructureDiscovery:true});
assert.deepEqual(fitascDiscovered.expectedTargetsByPost,fitascVariableCounts,'FITASC reviewed stand structure is not coerced to fixed 25-target series');
const sessionPageSource = readFileSync('app/sessions/[id]/page.tsx','utf8');
assert.match(sessionPageSource,/isPostBasedScorecardImportDiscipline\(session\.discipline\)/,'session page exposes scorecard import to FITASC Sporting without changing target setup classification');
const manualResultSource = readFileSync('app/results/new/page.tsx','utf8');
assert.match(manualResultSource,/DISCIPLINE_OPTIONS/,'manual Competition result entry still uses canonical options containing FITASC Sporting');
...
const conflictGrid5=Array.from({length:5},(_,i)=>({postNumber:1,targetNumber:i+1,result:'hit',rawMark:'/',observedMarkCategory:'diagonal_stroke',confidence:'high',warning:null}));
let conflictRec=a.deriveCurrentPostReconciliation({currentCells:conflictGrid5,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:false});
assert.equal(conflictRec.reconciliationStatus,'conflict','5/5 reviewed grid conflicts with high-confidence 3/5 post total');
let conflictGrid4=a.applyUserCorrection(conflictGrid5,1,5,'miss');
conflictRec=a.deriveCurrentPostReconciliation({currentCells:conflictGrid4,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:false});
assert.equal(a.summarizeGrid(conflictGrid4).score,4,'single correction changes reviewed grid to 4/5');
assert.equal(conflictRec.reconciliationStatus,'conflict','4/5 still conflicts with high-confidence 3/5 total until post override');
assert.equal(a.scorecardApplyIssues({grid:conflictGrid4,postCount:1,reviewedPostNumbers:[],postStatuses:{1:conflictRec.reconciliationStatus}}).length,1,'apply gating blocks unresolved high-confidence total conflict');
let acceptedRec=a.deriveCurrentPostReconciliation({currentCells:conflictGrid4,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:true});
assert.equal(acceptedRec.reconciliationStatus,'needs_review','explicit post acceptance downgrades high-confidence total mismatch from conflict');
assert.equal(a.scorecardApplyIssues({grid:conflictGrid4,postCount:1,reviewedPostNumbers:[1],postStatuses:{1:acceptedRec.reconciliationStatus}}).length,0,'apply gating allows explicitly accepted reviewed post total');

const editedAfterAcceptance=a.applyUserCorrection(conflictGrid4,1,4,'miss');
const editedAfterAcceptanceRec=a.deriveCurrentPostReconciliation({currentCells:editedAfterAcceptance,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:false});
assert.equal(a.summarizeGrid(editedAfterAcceptance).score,3,'later edit can naturally match detected total');
assert.equal(editedAfterAcceptanceRec.reconciliationStatus,'matched','later edit matching detected total resolves without post override');
const editedStillConflicting=a.applyUserCorrection(conflictGrid4,1,5,'hit');
const editedStillConflictingRec=a.deriveCurrentPostReconciliation({currentCells:editedStillConflicting,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:false});
assert.equal(a.scorecardApplyIssues({grid:editedStillConflicting,postCount:1,reviewedPostNumbers:[],postStatuses:{1:editedStillConflictingRec.reconciliationStatus}}).length,1,'editing an accepted conflicting post invalidates acceptance and blocks apply again');
const reacceptedRec=a.deriveCurrentPostReconciliation({currentCells:editedStillConflicting,detectedPostScore:3,detectedPostScoreConfidence:'high',expectedTargetCount:5,explicitlyReviewed:true});
assert.equal(a.scorecardApplyIssues({grid:editedStillConflicting,postCount:1,reviewedPostNumbers:[1],postStatuses:{1:reacceptedRec.reconciliationStatus}}).length,0,'explicitly accepting the new reviewed total allows apply again');
const provenanceGrid=[{postNumber:1,targetNumber:1,result:'hit',rawMark:null,confidence:'medium',warning:'Resolved from high-confidence post total.'},{postNumber:1,targetNumber:2,result:'miss',rawMark:null,confidence:'medium',warning:'Resolved from post total and mark evidence.'}];
assert.equal(a.unresolvedWholeScorecardItems({grid:provenanceGrid,postCount:1,postStatuses:{}}).length,0,'safely reconciled provenance warnings are not unresolved blockers');
assert.equal(a.scorecardApplyIssues({grid:provenanceGrid,postCount:1,reviewedPostNumbers:[],postStatuses:{}}).length,0,'apply gating agrees provenance warnings are nonblocking');
const trueUnknown=[{postNumber:1,targetNumber:1,result:'unknown',cellState:'uncertain',rawMark:null,confidence:'low',warning:'ambiguous'}];
assert.equal(a.unresolvedWholeScorecardItems({grid:trueUnknown,postCount:1,postStatuses:{}}).length,1,'true unknown/uncertain appears in attention list');
assert.equal(a.scorecardApplyIssues({grid:trueUnknown,postCount:1,reviewedPostNumbers:[],postStatuses:{1:'needs_review'}}).length,1,'true unknown/uncertain blocks apply');
assert.equal(a.unresolvedWholeScorecardItems({grid:conflictGrid4,postCount:1,postStatuses:{1:'conflict'}}).length,1,'post conflict appears in attention list');
assert.equal(a.scorecardApplyIssues({grid:conflictGrid4,postCount:1,reviewedPostNumbers:[],postStatuses:{1:'conflict'}}).length,1,'post conflict blocks apply');
assert.equal(a.unresolvedWholeScorecardItems({grid:conflictGrid4,postCount:1,postStatuses:{1:acceptedRec.reconciliationStatus}}).length,0,'explicit resolution removes conflict from attention list');
assert.equal(a.scorecardApplyIssues({grid:conflictGrid4,postCount:1,reviewedPostNumbers:[1],postStatuses:{1:acceptedRec.reconciliationStatus}}).length,0,'explicit resolution removes conflict from apply blockers');
assert.equal(a.unresolvedWholeScorecardItems({grid:trueUnknown,postCount:1,postStatuses:{1:'needs_review'}}).length > 0, a.scorecardApplyIssues({grid:trueUnknown,postCount:1,reviewedPostNumbers:[],postStatuses:{1:'needs_review'}}).length > 0, 'whole-card attention and apply gating agree on blockers');

const cursorGrid=[{postNumber:1,targetNumber:1,result:'unknown',rawMark:null,confidence:'low',warning:null},{postNumber:1,targetNumber:2,result:'unknown',rawMark:null,confidence:'low',warning:null},{postNumber:2,targetNumber:1,result:'unknown',rawMark:null,confidence:'low',warning:null},{postNumber:3,targetNumber:1,result:'miss',rawMark:null,confidence:'high',warning:null}];
let next=a.nextWholeScorecardReviewItem({grid:cursorGrid,postCount:3,postStatuses:{},after:null}); assert.deepEqual([next.postNumber,next.targetNumber],[1,1],'uncertain cursor starts at first unresolved item');
next=a.nextWholeScorecardReviewItem({grid:cursorGrid,postCount:3,postStatuses:{},after:next}); assert.deepEqual([next.postNumber,next.targetNumber],[1,2],'uncertain cursor visits multiple unresolved cells on same post');
next=a.nextWholeScorecardReviewItem({grid:cursorGrid,postCount:3,postStatuses:{},after:next}); assert.deepEqual([next.postNumber,next.targetNumber],[2,1],'uncertain cursor continues across posts');
next=a.nextWholeScorecardReviewItem({grid:cursorGrid,postCount:3,postStatuses:{},after:next}); assert.deepEqual([next.postNumber,next.targetNumber],[1,1],'uncertain cursor wraps after final unresolved item');
const resolvedCurrent=a.applyUserCorrection(cursorGrid,1,1,'hit');
next=a.nextWholeScorecardReviewItem({grid:resolvedCurrent,postCount:3,postStatuses:{},after:{postNumber:1,targetNumber:1}}); assert.deepEqual([next.postNumber,next.targetNumber],[1,2],'resolved current item disappears before continuing');
let uiCursorAfterResolved=a.nextWholeScorecardReviewItem({grid:a.applyUserCorrection(cursorGrid,1,2,'hit'),postCount:3,postStatuses:{},after:{postNumber:1,targetNumber:2}}); assert.deepEqual([uiCursorAfterResolved.postNumber,uiCursorAfterResolved.targetNumber],[2,1],'UI cursor continues forward from last focused item after that item disappears');

const pageSource = readFileSync('app/sessions/[id]/scorecard-import/page.tsx','utf8'); assert.match(pageSource,/from\("session_post_targets"\)/, 'scorecard import UI loads session_post_targets'); assert.match(pageSource,/targetDefinitionsError/, 'scorecard import UI tracks target definition load failures'); assert.match(pageSource,/formatScorecardSetupSummary/, 'scorecard import UI renders the resolved detailed or compact setup summary'); assert.match(pageSource,/structureDetailsOpen/, 'structure editor is collapsed by default behind explicit edit state'); assert.match(pageSource,/wholeScorecardReview/, 'whole-card scorecard review is the default surface'); assert.match(pageSource,/cycleScorecardCellResult/, 'target cells expose fast direct cycling controls'); assert.match(pageSource,/Review uncertain only/, 'uncertain-only review action is available'); assert.match(pageSource,/Use reviewed total for this post/, 'detailed review exposes explicit post-total acceptance'); assert.match(pageSource,/invalidateAcceptedPosts/, 'post edits invalidate previous explicit acceptance'); assert.match(pageSource,/structureExceptionsSummary/, 'compact structure summary supports default plus exceptions copy');
