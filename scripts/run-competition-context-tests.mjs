...
const entryPaths = ["app/results/new/page.tsx", "app/results/quick/page.tsx", "app/sessions/[id]/scorecard-import/page.tsx", "app/import/leirdue/page.tsx", "app/results/CompetitionResultClaims.tsx"];
for (const path of entryPaths) assert.match(readFileSync(path, "utf8"), /#competition-context/, `${path} navigates to the context section after save`);
const contextTarget = page.match(/<section[^>]+id="([^"]+)"[^>]*aria-labelledby="([^"]+)"/);
assert.deepEqual(contextTarget?.slice(1), ["competition-context", "competition-context-heading"], "the post-save fragment resolves to the labelled context section");
