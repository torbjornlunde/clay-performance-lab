...
const context = buildLeirdueFieldContext(session, duplicateRows, [...duplicateRows, { ...rows[0], event_id: 'e2', event_title: 'Another event' }, { ...rows[0], event_id: 'e2', event_title: 'Another event' }]);
...
assert.equal(context.medianScore, 72.5, 'duplicate rows do not inflate median context');
...
assert.equal(known0.eventCount, 2, 'duplicate rows do not inflate recurring shooter counts');
