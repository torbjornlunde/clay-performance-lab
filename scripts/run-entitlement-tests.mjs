...
assert.equal(canUseFeature({ billingMode: 'beta_hidden', isApprovedBetaUser: true, plan: 'free' }, 'ai.coach_report_summary').allowed, true, 'approved beta user can use AI during hidden beta');
assert.equal(canUseFeature({ billingMode: 'beta_hidden', isApprovedBetaUser: false, plan: 'free' }, 'ai.coach_report_summary').allowed, false, 'non-approved beta-hidden user cannot use AI');
assert.equal(canUseFeature({ billingMode: 'beta_hidden', plan: 'free' }, 'coach_report.generate').allowed, false, 'missing beta approval does not grant Pro feature access');
