...
const headers = { id: 'msg_1', timestamp: '1788771600', signature: 'v1,test' };
...
assert.deepEqual(verificationInput, { payload: eventBody, headers, webhookSecret: 'whsec_test' }, 'raw body, signature headers, and secret are passed to the official Resend verification boundary');
await assert.rejects(() => webhook.verifyAndParseResendWebhook({ payload: eventBody, headers, secret: 'whsec_test' }, async () => { throw new Error('invalid'); }), /invalid/, 'invalid signature rejection propagates before event processing');
assert.equal(webhook.deliveryStatusForEvent(delivered.type), 'delivered', 'delivered webhook becomes delivered');
for (const [type, status] of [['email.bounced', 'bounced'], ['email.failed', 'failed'], ['email.suppressed', 'suppressed']]) {
  const parsed = webhook.parseResendDeliveryEvent({ type, created_at: '2026-09-07T09:02:00Z', data: { email_id: 'email_123' } }, `msg_${status}`);
...
assert.match(adminRoute, /emailStatus: "accepted",\s*trackingStatus: "failed"/, 'tracking persistence failure preserves provider acceptance as a distinct result');
