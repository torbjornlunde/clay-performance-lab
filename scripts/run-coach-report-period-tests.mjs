...
assert(report.plainText.includes('Basic coach report'), 'report labels deterministic basic report');
assert(report.plainText.includes('Coach summary'), 'report includes coach summary');
assert(report.plainText.includes('What the data shows'), 'report includes data interpretation section');
assert(report.plainText.includes('Main focus areas'), 'report includes focus areas');
assert(report.plainText.includes('Recommended training'), 'report includes training recommendations');
assert(report.plainText.includes('Data quality and missing information'), 'report includes data quality section');
...
assert(report.plainText.includes('best was'), 'best score summary is included');
assert(report.plainText.includes('weakest was'), 'weakest score summary is included');
assert.match(report.plainText, /Behind misses.*2 misses.*verification drill/s, 'repeated miss patterns include count and useful explanation');
assert(report.sections.some((section) => section.title === 'Session notes/context'), 'notes-based context section can be included');
...
assert(report.plainText.includes('Notes context is available but turned off'), 'notes-based context can be turned off');
...
for (const text of ['Coach report', 'From date', 'To date', 'Include notes-based context', 'Raw private notes are not shown', 'Update report preview', 'Report preview needs update', 'Basic coach report', 'Copy visible report', 'Copied', 'Review the report below before copying it.']) assert(page.includes(text), `/coach-report page includes ${text}`);
