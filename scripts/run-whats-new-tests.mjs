...
assert.deepEqual(groups.map(({ heading }) => heading), ["September 2026", "August 2026", "July 2026"], "English month headings are derived newest-first");
assert.deepEqual(groups[0].entries.map(({ id }) => id), ["v29.09.26", "v28.09.26", "v27.09.26", "v26.09.26", "v25.09.26", "v24.09.26", "v23.09.26", "v22.09.26", "v21.09.26", "v20.09.26", "v19.09.26", "v18.09.26", "v17.09.26", "v16.09.26", "v15.09.26", "v14.09.26", "v13.09.26", "v12.09.26", "v11.09.26", "v10.09.26", "v9.09.26", "v8.09.26", "v7.09.26", "v6.09.26", "v5.09.26", "v4.09.26", "v3.09.26", "v2.09.26", "v1.09.26"], "September sequence is newest first without gaps");
assert.deepEqual(groups[1].entries.map(({ id }) => id), ["v14.08.26", "v13.08.26", "v12.08.26", "v11.08.26", "v10.08.26", "v9.08.26", "v8.08.26", "v7.08.26", "v6.08.26", "v5.08.26", "v4.08.26", "v3.08.26", "v2.08.26", "v1.08.26"], "newest update is first within month");
assert.equal(updates.latestWhatsNewEntry(entries).id, "v29.09.26", "latest update is deterministic");
...
assert.equal(seen.isLatestWhatsNewUnseen("v6.08.26", "v6.08.26"), false, "latest seen value hides badge");
assert.equal(seen.isLatestWhatsNewUnseen("broken", "v6.08.26"), true, "malformed stored value is safely unseen");
...
assert.doesNotThrow(() => seen.readWhatsNewUnseen(throwingStorage, "v6.08.26"));
assert.equal(seen.readWhatsNewUnseen(throwingStorage, "v6.08.26"), false, "failed storage read avoids a permanent broken badge");
assert.doesNotThrow(() => seen.markLatestWhatsNewSeen(throwingStorage, "v6.08.26"));
let stored = null;
let eventName = "";
...
assert.equal(seen.markLatestWhatsNewSeen(memoryStorage, "v5.08.26", eventTarget), true, "opening page marks latest seen");
assert.equal(stored, "v5.08.26");
assert.equal(eventName, seen.WHATS_NEW_SEEN_EVENT, "same-tab synchronization event is dispatched");
assert.equal(seen.readWhatsNewUnseen(memoryStorage, "v5.08.26"), false, "same-tab refresh removes badge");
