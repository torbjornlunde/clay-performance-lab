...
assert.deepEqual(groups[0].entries.map(({ id }) => id), ["v3.08.26", "v2.08.26", "v1.08.26"], "newest update is first within month");
assert.equal(updates.latestWhatsNewEntry(entries).id, "v3.08.26", "latest update is deterministic");

assert.equal(seen.isLatestWhatsNewUnseen(null, "v3.08.26"), true, "empty seen value is unseen");
...
assert.doesNotThrow(() => seen.markLatestWhatsNewSeen(throwingStorage, "v3.08.26"));
let stored = null;
let eventName = "";
...
assert.equal(seen.markLatestWhatsNewSeen(memoryStorage, "v3.08.26", eventTarget), true, "opening page marks latest seen");
assert.equal(stored, "v3.08.26");
assert.equal(eventName, seen.WHATS_NEW_SEEN_EVENT, "same-tab synchronization event is dispatched");
assert.equal(seen.readWhatsNewUnseen(memoryStorage, "v3.08.26"), false, "same-tab refresh removes badge");
