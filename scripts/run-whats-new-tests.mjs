...
assert.deepEqual(groups[0].entries.map(({ id }) => id), ["v5.08.26", "v4.08.26", "v3.08.26", "v2.08.26", "v1.08.26"], "newest update is first within month");
assert.equal(updates.latestWhatsNewEntry(entries).id, "v5.08.26", "latest update is deterministic");

assert.equal(seen.isLatestWhatsNewUnseen(null, "v5.08.26"), true, "empty seen value is unseen");
assert.equal(seen.isLatestWhatsNewUnseen("v4.08.26", "v5.08.26"), true, "older seen value is unseen");
assert.equal(seen.isLatestWhatsNewUnseen("v5.08.26", "v5.08.26"), false, "latest seen value hides badge");
assert.equal(seen.isLatestWhatsNewUnseen("broken", "v5.08.26"), true, "malformed stored value is safely unseen");
...
assert.doesNotThrow(() => seen.readWhatsNewUnseen(throwingStorage, "v5.08.26"));
assert.equal(seen.readWhatsNewUnseen(throwingStorage, "v5.08.26"), false, "failed storage read avoids a permanent broken badge");
assert.doesNotThrow(() => seen.markLatestWhatsNewSeen(throwingStorage, "v5.08.26"));
let stored = null;
let eventName = "";
