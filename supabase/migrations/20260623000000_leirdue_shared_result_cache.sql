...
create unique index if not exists leirdue_shared_results_identity_uidx on public.leirdue_shared_shooter_results (year, coalesce(event_id, ''), coalesce(liste_id, ''), source_url, normalized_name, coalesce(discipline, ''), coalesce(event_date, '1900-01-01'::date), coalesce(score, -1), coalesce(total_targets, -1));
...
insert into public.leirdue_shared_shooter_results (
  ...
  raw_row, validation_status, parser_version, parsed_at, updated_at
)
select
  year,
  event_id,
