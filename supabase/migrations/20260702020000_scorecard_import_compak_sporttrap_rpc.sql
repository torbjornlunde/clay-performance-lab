...
begin
  ...
  if array_length(p_targets_per_post_by_post,1) <> p_post_count then raise exception 'dimension_mismatch'; end if;
  if lower(coalesce(v_session.discipline,'')) = 'compak sporting' then
