create function public.apply_scorecard_import_v2(
  p_session_id uuid,
...
begin
  ...
  if coalesce(v_session.post_count, v_session.course_count) <> p_post_count or v_session.targets_per_post <> p_targets_per_post or array_length(p_targets_per_post_by_post,1) <> p_post_count then raise exception 'dimension_mismatch'; end if;
  for v_course in 1..p_post_count loop
    v_expected_count := p_targets_per_post_by_post[v_course];
    if v_expected_count is null or v_expected_count < 1 then raise exception 'dimension_mismatch'; end if;
...
revoke all on function public.apply_scorecard_import_v2(uuid,uuid,text,integer,integer,integer[],integer,integer,jsonb,boolean,integer,jsonb) from public, anon;
grant execute on function public.apply_scorecard_import_v2(uuid,uuid,text,integer,integer,integer[],integer,integer,jsonb,boolean,integer,jsonb) to authenticated;
