...
create table if not exists public.session_target_definitions(id uuid primary key default gen_random_uuid(),session_id uuid not null references public.sessions(id) on delete cascade,course_number integer not null,machine text not null,target_type text,direction text,speed text,distance text,difficulty text,notes text,created_at timestamptz default now(),unique(session_id,course_number,machine));
