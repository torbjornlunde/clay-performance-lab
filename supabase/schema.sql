...
create table if not exists public.session_courses(id uuid primary key default gen_random_uuid(),session_id uuid not null references public.sessions(id) on delete cascade,course_number integer not null,fitasc_scheme integer,compak_programme_type text,compak_conflict_resolution text,shooter_number integer,start_plate integer,created_at timestamptz not null default now(),constraint session_courses_compak_programme_type_check check (compak_programme_type is null or compak_programme_type in ('five_singles','three_singles_one_report_pair','three_singles_one_simultaneous_pair','one_single_two_report_pairs','one_single_two_simultaneous_pairs')),constraint session_courses_compak_conflict_resolution_check check (compak_conflict_resolution is null or compak_conflict_resolution in ('exact_authoritative','remembered_discrepancy')),constraint session_courses_compak_resolution_has_sources_check check (compak_conflict_resolution is null or (fitasc_scheme is not null and compak_programme_type is not null)));
