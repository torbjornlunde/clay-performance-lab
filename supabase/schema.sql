...
create table if not exists public.sessions(id uuid primary key default gen_random_uuid(),user_id uuid not null references auth.users(id) on delete cascade,name text not null,discipline text not null,session_type text not null,shooting_format text,course_count integer,total_targets integer,own_score integer,winning_score integer,notes text,leirdue_result_url text,created_at timestamptz not null default now());
